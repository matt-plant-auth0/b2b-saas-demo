...
function EditRoles({ id }) {
  ...
  //   return (
  //     <>
  //       <button key={id} onClick={(event) => setViewMode(!viewMode)}>
  //         <svg
  //           xmlns="http://www.w3.org/2000/svg"
  //           width="24"
  //           height="24"
...
export default function Members({ members, invites, allRoles }) {
  ...
  function toggleViewMode(user_id, viewOnly) {
    console.log("toggleViewMode user_id", user_id);
    console.log("toggleViewMode viewOnly", viewOnly);
    ...
    console.log("toggleViewMode viewMembers", viewMembers);
  }

  async function handleUpdateRole(value, action) {
    console.log("handleUpdateRole value", value);
    console.log("handleUpdateRole action", action);
    if (action.action == "remove-value") {
      ...
      try {
        ...
        if (res.ok) {
          console.log("handleUpdate - add - viewMembers", viewMembers);
...
export const getServerSideProps = withPageAuthRequired({
  async getServerSideProps({ req, res }) {
    // console.log("Mgmt API ===> list org members ");
    //fetch connections
